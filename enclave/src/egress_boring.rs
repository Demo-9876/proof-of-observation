...
use btls::ssl::{HandshakeError, SslConnector, SslMethod, SslStream, SslVerifyMode, SslVersion};
use btls::x509::X509;
...
pub fn connect<S: Read + Write>(
    ...
    cfg.connect(sni, sock).map_err(|e| match e {
        HandshakeError::SetupFailure(e) => format!("btls TLS 握手初始化失败: {e}"),
        HandshakeError::Failure(e) => format!("btls TLS 握手失败: {}", e.error()),
        HandshakeError::WouldBlock(_) => "btls TLS 握手未完成".to_string(),
    })
}
