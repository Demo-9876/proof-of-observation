...
如果你要从零跑“代理版本 Enclave”全流程，请直接看独立文档：

[docs/qingtian-forward-proxy-e2e-runbook.md](qingtian-forward-proxy-e2e-runbook.md)

本节只保留补充说明：

- Enclave 仍然通过 `qproxy enclave` 出网。
- 父 VM 侧可以切到 `HTTP CONNECT` 或 `SOCKS5` 正向代理。
- 代理版的完整启动、请求、验证、停止步骤都已经拆到独立 runbook。
- quickstart 这里继续保留的是前面标准 qproxy E2E 的主流程。
